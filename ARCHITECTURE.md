...
- Navigate frame-by-frame: `←` / `→`, mouse wheel, `<<` / `<` / `>` / `>>` buttons, `Space` or Play / Stop, click on either timeline. `Shift`+arrow and `<<` / `>>` jump by `jump_seconds` worth of frames. All key bindings live in `gui.ui_components._bind_navigation` and are suppressed while the note or frame entry has focus (`_guard_key`). Wheel notches are queued and consumed one frame per video frame interval (`LabelingApp._wheel_tick`), so a fast flick moves at playback speed rather than racing the redraw; the queue is capped at `WHEEL_BACKLOG_S` so motion stops shortly after the wheel does.
- The current frame number is editable: focus pauses playback and cancels queued wheel/arrow movement; Enter validates and jumps through `next_frame`, while Escape or focus loss cancels. Draft text is separate from `video.current_frame`, so redraws cannot overwrite it. The last-frame index stays read-only; Notes is only for annotations.
